...
void DrawBillboardTree()
{
	...
	for (int i = 0; i < maxBillboardTree; i++)
	{
		...
		matrixWorld *= XMMatrixScaling(28.0f, 64.0f, 28.0f);
		...
		// 移動マトリクス。gpuで計算されている。
		matrixWorld *= XMMatrixTranslation(g_BillboardTree[i].position.x, g_BillboardTree[i].position.y, g_BillboardTree[i].position.z);
