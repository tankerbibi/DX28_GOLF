...
struct BillboardTree
{
	XMFLOAT3 position;
	bool use;
};

static constexpr int maxBillboardTree = 500;
...
static BillboardTree g_BillboardTree[maxBillboardTree];

void InitializeBillboardTree()
{
	{
		// 頂点バッファの作成
...

	{
		...
		desc.ByteWidth = sizeof(InstanceData) * maxBillboardTree * 2;
		// 毎フレーム更新するため動的に設定
	...
	for (int i = 0; i < maxBillboardTree; i++)
	{
		g_BillboardTree[i].position = { 0.0f,0.0f,0.0f };
		g_BillboardTree[i].use = false;
...
void UpdateBillboardTree()
{
	// 特に何もしない
}

// 座標を指定して描画する関数に変更
void DrawBillboardTree()
{
	...
	for (int i = 0; i < maxBillboardTree; i++)
	{
		if (g_BillboardTree[i].use == false) continue;

		const XMFLOAT3 cameraPos = GetCameraPosition();

		const XMVECTOR tPos = XMLoadFloat3(&g_BillboardTree[i].position);
		const XMVECTOR cPos = XMLoadFloat3(&cameraPos);

		XMVECTOR forward = XMVectorSubtract(cPos, tPos);
		...
		XMMATRIX matrixWorld = XMMatrixScaling(28.0f, 64.0f, 28.0f);
		matrixWorld *= rot;
		matrixWorld *= XMMatrixRotationY(XM_PI);
		// XMMatrixTranslationと同じ
		matrixWorld *= XMMatrixTranslationFromVector(tPos);
...
void CreateBillboardTree(XMFLOAT3 position)
{
	for (int i = 0; i < maxBillboardTree; i++)
	{
		if (g_BillboardTree[i].use == true) continue;
		g_BillboardTree[i].position = position;
		g_BillboardTree[i].use = true;
		break;
	}
}
