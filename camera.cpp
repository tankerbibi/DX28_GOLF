...
static CameraMode g_CameraMode = CameraMode::PLAY;

static XMFLOAT3 g_FixCameraPosition[3]{
	{-0.5f, 5.0f, 0.0f}, {5.0f, 5.0f, 5.0f}, {10.0f, 10.0f, 10.0f} };
...
void DrawCamera() {
	...
	g_ViewMatrix =
		XMMatrixLookAtLH(XMLoadFloat3(&g_Position),
			XMLoadFloat3(&g_CameraTargetPos), XMLoadFloat3(&up));
	...
	g_ProjectionMatrix = XMMatrixPerspectiveFovLH(
		...
		static_cast<float>(screenHeight), // Aspect ratio
...
void DrawCameraMap() {
	...
	const XMFLOAT3 up =
		XMFLOAT3(0.0f, 0.0f, 1.0f); // カメラの上方向 いつもと違うよ！
	g_ViewMatrix = XMMatrixLookAtLH(XMLoadFloat3(&position),
		XMLoadFloat3(&target), XMLoadFloat3(&up));

	// プロジェクションマトリクス 3dの画面に大きさを合わせるためのマトリクス
	// （新規）
	g_ProjectionMatrix =
		XMMatrixPerspectiveFovLH(1.3f, // Field of view angle in radians
...
XMMATRIX GetCameraViewMatrix() { return g_ViewMatrix; }

XMMATRIX GetCameraProjectionMatrix() { return g_ProjectionMatrix; }
...
XMFLOAT3 GetCameraPosition() { return g_Position; }

void SetCameraMode(CameraMode newCameraMode) { g_CameraMode = newCameraMode; }
...
void ReflectsDebugKeyOperations() 
{
	...
	XMMATRIX rotationMatrix =
		XMMatrixRotationRollPitchYaw(g_CameraPitch, g_CameraYaw, 0.0f);
	XMVECTOR forwardVec_WS =
		XMVector3TransformNormal(forwardBase, rotationMatrix);

	// --- 2. A/D (左右) 用の"水平"前方ベクトルを計算 (ピッチを 0.0f にする) ---
	XMMATRIX horizontalRotationMatrix =
		XMMatrixRotationRollPitchYaw(0.0f, g_CameraYaw, 0.0f);
	XMVECTOR forwardVec_AD =
		XMVector3TransformNormal(forwardBase, horizontalRotationMatrix);
	...
	{
		XMMATRIX leftRotationMatrix = XMMatrixRotationRollPitchYaw(
			0.0f, -XM_PIDIV2, 0); // 回転マトリクスを取得
		XMVECTOR left = XMVector3TransformNormal(
			forwardVec_AD, leftRotationMatrix); // 前方ベクトルを回転
		velocity = XMVectorAdd(velocity, left);
	}
	else if (Keyboard_IsKeyDown(KK_D)) 
	{
		XMMATRIX rightRotationMatrix = XMMatrixRotationRollPitchYaw(
			0.0f, XM_PIDIV2, 0); // 回転マトリクスを取得
		XMVECTOR right = XMVector3TransformNormal(
			forwardVec_AD, rightRotationMatrix); // 前方ベクトルを回転
		velocity = XMVectorAdd(velocity, right);
	}

	if (Keyboard_IsKeyDown(KK_E))
	{
		XMMATRIX upRotationMatrix =
			XMMatrixRotationRollPitchYaw(-XM_PIDIV2, 0, 0); // 回転マトリクスを取得
		XMVECTOR up = XMVector3TransformNormal(
			forwardBase, upRotationMatrix); // 前方ベクトルを回転
		velocity = XMVectorAdd(velocity, up);
	}
	else if (Keyboard_IsKeyDown(KK_Q)) 
	{
		XMMATRIX downRotationMatrix =
			XMMatrixRotationRollPitchYaw(XM_PIDIV2, 0, 0); // 回転マトリクスを取得
		XMVECTOR down = XMVector3TransformNormal(
			forwardBase, downRotationMatrix); // 前方ベクトルを回転
	...
	velocity = XMVectorScale(velocity, speed);
	...
	XMVECTOR targetPosVec = XMVectorAdd(
		XMLoadFloat3(&g_Position), forwardVec_WS);   // 注視点ベクトルを導き出す
	XMStoreFloat3(&g_CameraTargetPos, targetPosVec); // 注視点ベクトルを座標に変換
}
