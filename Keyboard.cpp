...
static Keyboard_State gOldState = {};
...
bool Keyboard_IsKeyTrigger(Keyboard_Keys key, const Keyboard_State* pState, const Keyboard_State* pOldState)
{
    if (key <= 0xfe)
    {
        unsigned int* p = (unsigned int*)pState;
...
{
    return Keyboard_IsKeyTrigger(key, &gState, &gOldState);
}
