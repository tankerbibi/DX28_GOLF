...
float4 main(in float4 position : POSITION0,
            ...
            //in row_major float4 mtxInstanceRow0 : TEXCOORD1,
            //in row_major float4 mtxInstanceRow1 : TEXCOORD2,
            //in row_major float4 mtxInstanceRow2 : TEXCOORD3,
            //in row_major float4 mtxInstanceRow3 : TEXCOORD4,
            in row_major float4x4 mtxInstance : INSTANCE, // ★ここに追加！ (Slot 1から来るデータ)
            ...
            out float4 outColor : COLOR0) : SV_Position  //inは入力用の引数。　POSITION0、TEXCOORD0はセマンティクス。変数の役割を設定できる。  outは出力用の引数。
{
    ...
    {
        // 法線ベクトルの座標変換(回転)
        float3 worldNormal = mul(float4(N, 0.0), mtxInstance).xyz;  // normalを回転させる
    ...
    float4 worldPos = mul(position, mtxInstance);
